...
zrd = zeros(256,100);
zrd(1:127,:) = rd; % zero padding

fft_zrd = fft(zrd); % calcuating the fft

hm = fftshift(hamming(256)); % creating a hamming window

%creating filter
h = zeros(256,1);
for i=2:2:256
    h(i) = (-1/2)*(alpha/(pi*alpha*sin((i-129)*alpha)))^2;
end
h(129) = 1/(8*alpha^2);
ha = abs(fft(h))*alpha;
...
Qr = real(Q(1:127,:)); % retaining only real values and removing zero padding
